...
WorldState make_world(content::LevelDefinition level) {
    ...
    for (const auto& object : world.level.root_objects) {
        world.entities.push_back(EntityState{
            next_id++, object.name, {}, object.position, object.layer, object.visible});
    }
    for (const auto& room : world.level.rooms) {
        ...
        for (const auto& object : room.objects) {
            world.entities.push_back(EntityState{
                next_id++, object.name, room.id, object.position, object.layer, object.visible});
        }
        for (const auto& door : room.doors) {
            world.entities.push_back(EntityState{
                next_id++, door.id, room.id, door.position, door.layer, door.visible});
        }
    }
    return world;
}
